...
class worker_pool():
    def __init__(self, sim_info: dict, num_workers: int):
        ...
        sim_info = DictConfig({'size': sim_info['size'],'obj_fns': sim_info['obj_fns'] ,'sh_path':  'Simulations/simulation_trigger.sh'}) # Path is relative to the /src folder
        self.workers = ActorPool([r_sim_client.remote(sim_info) for _ in range(self.num_workers)])  
        
    def kill_client(self):
        self.workers.map(lambda worker, kill: worker.terminate_client.remote(kill), [None for _ in range(self.num_workers)])
