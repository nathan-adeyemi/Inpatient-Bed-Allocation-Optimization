...
class solution_set:
    def __init__(self, sol_set: list = [], obj_fns: list = []):
        self.set = [sol for sol in sol_set if sol is not None]
            ...
            for j in range(len(self.set)):
                ...
                if  self.set[i] is not self.set[j]:
                    crit_1 = self.set[i].soln_comparison(
                        compare_sol=self.set[j], stat=noisy
                    )
                    crit_2 = self.set[j].soln_comparison(
                        compare_sol=self.set[i], stat=noisy
                    )
