...
class pareto_set(solution_set):
    def __init__(self,stoch_optim = True, set: list = None, obj_fns: list = None):
        super().__init__(sol_set = set,
                         obj_fns = obj_fns)
    ...
    def find_g_ideal(self):
        ...
        self.g_ideal = pd.concat([pd.concat([self.set[idxs[self.obj_fns_names.index(col)]].data.loc[:,col] for _ in range(int(reps[self.obj_fns_names.index(col)]))]) for col in self.obj_fns_names],axis=1)
...
class candidate_set(solution_set):
    def __init__(self, workers: worker_pool,
                 candidates: list = [],
                 obj_fns: list = []):
    ...
    def generate_samples(self):
        ...
        results = self.worker_pool.workers.map(lambda worker, allocation: worker.generate_samples.remote(allocation), 
