...
class dd_pusa(popMultiObjOptim):
    ...
    def create_workers(self):
        self.worker_pool = worker_pool(num_workers=self.num_workers,
    ...
    def generate_candidate(self):
        ...
        while True:
            ...
            if self.pareto_set.best is not None:
                ...
                new_sol = np.array([fix_cell(i, lim=self.tweak_limit * 3) for i in (self.pareto_set.best.solution + changes)])
            else:
    ...
    def update_history(self,return_val: bool = False):
        if return_val:
            ...
            self.history.append({
                ...
                'Estimated Pareto Front Obj Fn (all fns):': pd.concat([i.data.mean(axis=0) for i in self.pareto_set.set],axis = 1).T})
    ...
    def execute_iteration(self):
        ...
        candidate_solutions = candidate_set(candidates = [self.generate_candidate() for _ in range(self.sols_per_iter)],
                                                 workers=self.worker_pool,
                                                 obj_fns = self.obj_fns)
        candidate_solutions.generate_samples()
        self.sample_counter = np.sum(np.array([sol.replications for sol in candidate_solutions.set]))
        if len(candidate_solutions.set) > 0:
            if self.use_moocba:
                self.mo_ocba_reps = 0
                candidate_solutions, self.rejects = self.mo_ocba(candidate_solutions)
    ...
    def __reduce__(self):
        ...
        return (self.__class__, (self.experiment_info,
                                 self.hyper_params,
