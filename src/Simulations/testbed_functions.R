...
TB_obj_1 <- function(x) {
  ...
  ][,.(TB_obj_1 = weighted.mean(server_util,na.rm = T)), by = replication]
  return(z)
}
...
TB_obj_2 <- function(x){
  ...
  return(
    ...
    ][,.(TB_obj_2 = mean(wait_time,na.rm = T)),by = replication])
}

TB_obj_3 <- function(x) {
  if (is.list(x)) {
    z <-
      x[[2]][, .(queue_length = mean(system)), by = list(resource, replication)][, .(TB_obj_3 = max(queue_length)), by = replication]
    # return(x[[2]][,mean_server_utilization <- mean(utilization,na.rm = T), by  = replication]
  } else{
    z <-
      x[, .(TB_obj_3 = sd(activity_time, na.rm = T)), by = list(replication)]
  }
  return(z)
}
...
run_testbed_sim <-
  function(rep_nums = NA,
    ...
    set.seed(seed)
    setDT(network_df)
    network_df[,server_count := solution]
